         ...
         case task.action.toLower():
            ...
            of "upload":
               ...
               if (jtemp.ChunkNum != jtemp.TotalChunks) or (jtemp.ChunkNum == 0 and jtemp.TotalChunks == 0): 
                  ...
                  let resp = when defined(CRYPTO): await Fetch(curConfig, $(uploadJson), true) else: await Fetch(curConfig, encode(curConfig.PayloadUUID & $(uploadJson)), true)
                  when not defined(release):
                     echo "resp for upload: ", $(resp)
                  # let parsedJsonresp = parseJson(decode(resp)[36 .. ^1])
                  let parsedJsonresp = when defined(CRYPTO): parseJson(resp[36 .. ^1]) else: parseJson(decode(resp)[36 .. ^1]) 
